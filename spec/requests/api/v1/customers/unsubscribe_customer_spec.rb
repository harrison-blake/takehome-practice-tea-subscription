...
describe 'Unsubscribe Customer Endpoint' do
  describe 'Happy Path' do
    before :each do
      @customer = Customer.create!(first_name: 'Harrison', last_name: 'Blake', email: 'test')
      @test_sub = Subscription.create!(title: 'Summer Mellow', price: 4.99, status: true)
      ...
      @customer.subscriptions << @test_sub
    end
    ...
    it "subscribes a customer to a tea subscription" do
      ...
      VCR.use_cassette('unsubscribe_customer') do
        put "/api/v1/customers/#{@customer.id}", headers: headers, params: params.to_json
      end
      ...
      expect(customer[:data][:attributes][:subscriptions][0][:status]).to eq("false")
    end
  end
end
